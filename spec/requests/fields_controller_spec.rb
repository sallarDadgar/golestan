...
RSpec.describe 'fields Controller', type: :request do
  ...
  context 'invalid params' do
    context '.create' do
      it 'should not add field when valid attributes not provided' do
        currentprof = create(:admin)
        currentuser = create(:user, role: :admin, fkey: currentprof)
        prof = create(:prof)
        sign_in(currentuser)
        ...
        expect(json['unit']).to eql(["can't be blank"])
        expect(json['fkey']).to eql(["must exist"])
        expect(json['projor.prof']).to eql(["can't be blank"])
      end
    end
    context '.update' do
      it 'should update specified field' do
